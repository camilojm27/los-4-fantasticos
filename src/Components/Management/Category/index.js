...
import { addCategory, getCategory, editCategory, deleteCategory } from '../../../redux/actions/categoryAction'
...
const Category = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (modal.insert === true) {
      ...
      delete data.id
      dispatch(addCategory(data, currentUser.Authorization)).then(() => {
      ...
      if (modal.edit === true) {
        ...
        dispatch(editCategory(data, currentUser.Authorization)).then(() => {
          setModal({ remove: false, edit: false, insert: false })
          dispatch(getCategory())
        }).catch(error => {
